...
import { groupByChannelReal } from '~/api'
...
function getMessageListByPageNumb() {
  ...
  groupByChannelReal(param).then((res: any) => {
    tableData.value = res?.dailyConsultingList || []
    ...
    tableData.value.forEach((item: any) => {
      ...
      if (index < 0) {
        datasetByChannel.push({
          ...
          values: [item.consultingCount],
        })
      }
      else {
        datasetByChannel[index].values[0] = datasetByChannel[index].values[0] + item.consultingCount
      }
    })
    ...
    <el-container>
      <el-main>
        ...
        <el-table :data="tableData" height="85%">
          ...
          <el-table-column prop="consultingCount" label="数量" />
          <el-table-column prop="consultingDate" label="统计日期" />
        </el-table>
